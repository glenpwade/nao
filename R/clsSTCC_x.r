...
setGeneric(name="calc.Gt_x",
           ...
           def = function(stcc1Obj){
             ...
             G <- 0
             if(this$speedopt == corrspeedopt$gamma) { G <- 1-exp(-speed*st_c) }
             if(this$speedopt == corrspeedopt$gamma_std) { G <- 1-exp(-speed/sd(this@st)*st_c) }
             if(this$speedopt == corrspeedopt$eta) { G <- 1-exp(-exp(speed)*st_c) }
...
## -- loglik.stcc1_x() --####
setGeneric(name=".loglik.stcc1_x",
           valueClass = "numeric",
           ...
           def = function(optimpars,z,stcc1Obj){
             ...
             maxSpeed <- switch(this$speedopt,1000,(1000/sd(this@st)),30.0,0.30)
             ...
             Pt <- .calc.Pt_x(this)  # T x nr.corPars
...
## --- estimateSTCC1_x --- ####
setGeneric(name="estimateSTCC1_x",
           valueClass = "stcc1_class",
           ...
           def = function(z,stcc1Obj,estimationCtrl){
             ...
             tmp <- NULL
             try(tmp <- optim(optimpars,.loglik.stcc1_x,z,this,gr=NULL,method="BFGS",control=this$optimcontrol,hessian=calcSE))
             ...
             if (tmp$convergence==0) {
               ...
               this$Estimated$Pt <- .calc.Pt_x(this)
...
## --- estimateSTCC1_x --- ####
setMethod("estimateSTCC1_x",signature = c("matrix","stcc1_class","missing"),
          function(z,stcc1Obj){
            estimationControl <- list(calcSE = TRUE,verbose = TRUE)
            estimateSTCC1_x(z,stcc1Obj,estimationControl)
          })

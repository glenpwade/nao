...
setGeneric(name="test.CCCParsim",
           ...
           def = function(e,H0,st,testOrder){
             ...
             #for (n in 1:H0@N) {
              ...
              # if(h[,n] <- H0$ntvgarch[[n]]$garch$type != garchtype$noGarch){
             ...
             # Get x_garch
             x_garch <- .x_garch(w,H0,h,beta) # if none have GARCH, ncol(x_garch)=0
             ...
             # Get im_garch
             if (ncol(x_garch)!=0) im_garch <- .im_garch(H0,x_garch)
             if (ncol(x_garch)==0) im_garch <- matrix(nrow=0, ncol=0)
             ...
             # Get im_tv_garch
             Pt <- H0$Estimated$P
             if (ncol(x_garch)!=0) im_tv_garch <- .im_tv_garch(H0,Pt,x_tv,x_garch)
             if (ncol(x_garch)==0) im_tv_garch <- matrix(nrow=NROW(im_tv), ncol=0)
             ...
             # Get im_garch_cor
             if (ncol(x_garch)!=0) im_garch_cor <- .im_garch_cor_parsim(H0,x_garch,x_tau)
             if (ncol(x_garch)==0) im_garch_cor <- matrix(nrow=0, ncol=NCOL(im_cor))
...
setGeneric(name="test.CCCvSTCC1",
           ...
           def = function(e,H0,st,testOrder){
             ...
             # Get im_garch_cor
             if (ncol(x_garch)!=0) im_garch_cor <- .im_garch_cor(H0,x_garch,v_rho)
             if (ncol(x_garch)==0) im_garch_cor <- matrix(nrow=0, ncol=NCOL(im_cor))
